...
void suite("JSON resources", () => {
  ...
  void test("registers and retrieves resource by canonical URI", () => {
    const context = createContext();
    const node = { test: true };
    const resource = createResource("https://example.com/test.json", node);

    assert.equal(
      resolveResource(context, "https://example.com/test.json"),
      undefined,
    );
    ...
    assert.equal(
      resolveResource(context, "https://example.com/other.json"),
      undefined,
    );
  });

  void test("maintains both node and URI mappings", () => {
    const context = createContext();
    const node = { test: true };
    const resource = createResource("https://example.com/test.json", node);
    ...
    const byUri = resolveResource(context, "https://example.com/test.json");
    assert.equal(byNode, resource);
    assert.equal(byUri, resource);
    assert.equal(byNode, byUri);
  });

  void test("handles multiple resources in same context", () => {
    const context = createContext();
    const node1 = { id: 1 };
    const node2 = { id: 2 };
    ...
    assert.equal(
      resolveResource(context, "https://example.com/1.json"),
      resource1,
    );
    assert.equal(
      resolveResource(context, "https://example.com/2.json"),
      resource2,
    );
  });

  void test("handles resources without canonical URIs", () => {
    const context = createContext();
    const node1 = { id: 1 };
    const node2 = { id: 2 };
  ...
  void test("allows updating resource mappings", () => {
    const context = createContext();
    const node = { test: true };
    ...
    // Both mappings should be updated to the new resource
  ...
  void test("preserves anchors when resource is registered", () => {
    const context = createContext();
    const node = {};
    const resource = createResource("https://example.com/test.json", node);
    const anchor = { type: "string" };
    ...
    const retrieved = resolveResource(context, "https://example.com/test.json");
  ...
  void test("resolves canonical URIs", () => {
    const context = createContext();
    const node = { test: true };
    const resource = createResource("https://example.com/schema.json", node);
    setResource(context, resource);

    const result = resolveResourceUri(
      context,
      undefined,
  ...
  void test("resolves canonical URIs with pointer fragments", () => {
    const context = createContext();
    const node = {
      $defs: {
        string: { type: "string" },
  ...
  void test("resolves canonical URIs with anchor fragments", () => {
    const context = createContext();
    const node = {
      $defs: {
        string: { type: "string" },
  ...
  void test("resolves relative URI references", () => {
    const context = createContext();
    const baseNode = {};
  ...
  void test("resolves empty fragment against base resource", () => {
    const context = createContext();
    const node = { type: "string" };
    const resource = createResource(undefined, node);

    const result = resolveResourceUri(context, resource, "#");
    assert.equal(result, node);
  });

  void test("throws for unknown canonical URIs", () => {
    const context = createContext();
  ...
  void test("throws for relative URIs without base", () => {
    const context = createContext();

    assert.throws(() => {
      resolveResourceUri(context, undefined, "#/missing");
    }, ResolutionError);
  });

  void test("handles mixed pointer and anchor fragments", () => {
    const context = createContext();
    const node = {
      $defs: {
        user: {
    ...
    const nameType = resolveResourceUri(
      context,
      undefined,
    ...
    const email = resolveResourceUri(
      context,
      undefined,
